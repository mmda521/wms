...
class Biz extends CI_Controller {
    ...
    * */
    ...
    {
        $this->load->view('wms/biz');
		
    }
	
	
	//申报功能
	public function biz_report_send(){
        $control_id = $this->input->get_post("control_id");
        $condition=array('canshu'=>$control_id);
        header("Content-type:text/html; charset=utf-8");
        ini_set('soap.wsdl_cache_enabled', "0"); //关闭wsdl缓存
        $soap = new SoapClient('http://192.168.17.99:8090/BIZ.asmx?wsdl');
//获取配置扫描类型
    ...
    * */
     public function add()
    {   
		$condition = array('bus_record_id'=>1);
		$data['bus_list'] = $this->bus_record_model->get_list($condition);
        $this->load->view('wms/biz_add',$data);
    }
    ...
    public function doadd(){
        ...
        $col_code       = $this->input->get_post("col_code");
		 if(!empty($col_code)) {
            ...
            echo result_to_towf_new('1', 0, '资格编号不能为空', NULL);
            exit();
        }
        $trades_code   = $this->input->get_post("trades_code");
		 if(!empty($trades_code)) {
            ...
            echo result_to_towf_new('1', 0, '企业编号不能为空', NULL);
            exit();
        }
        ...
        $trades_name    = $this->input->get_post("trades_name");
        $control_flag = $this->input->get_post("control_flag");
        $app_person    = $this->input->get_post("app_person");
        ...
        $data = array(
            'col_code'      =>$col_code,
            'trades_code'  =>$trades_code,
			'biz_type'   =>$biz_type,			
            'trades_name'=>$trades_name,
    ...
    * */
     public function edit()
    {
     ...
     // PC::debug($list);	
	    $this->load->view('wms/biz_edit', $list);
...
/**
    ...
    * */
     public function do_edit()
    {
       ...
       $this->biz_model->update($data,$control_id);
	    echo result_to_towf_new('1', 1, 'success', null);
    }
    ...
    * */
    ...
    { 
		 ...
		 //PC::debug($GUID);
      if ($control_id) {
            $this->biz_model->delete($control_id);
