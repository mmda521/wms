 ...
 *
 **/
class Bus_record extends CI_Controller {
    ...
    * */
    public function ajax_data(){
        ...
        $condition = array();
		 ...
		 //获取检索号
        $enterprice_code = $this->input->get_post("enterprice_code");
        if(!empty($enterprice_code)){
            $condition['enterprice_code'] = $enterprice_code;
        }
		
        //获取库位号
        $enterprice_name = $this->input->get_post("enterprice_name");
        if(!empty($enterprice_name)){
            $condition['enterprice_name'] = $enterprice_name;
        }

        //启用状态
        $enterprice_type = $this->input->get_post("enterprice_type"); 
		 //PC::debug($status);
        if(in_array($enterprice_type,array('Y','N',true))){
            $condition['enterprice_type'] = $enterprice_type; 
        }
        ...
        $total = $this->bus_record_model->count_num($condition);      
        $page_string = $this->common_page->page_string($total, $limit, $page);

        $list = $this->bus_record_model->get_list($condition,$limit,$offset);
        foreach($list as $k=>$v){
            $list[$k]['enterprice_type'] = ($v['enterprice_type'] == '0' )?"公用型保税仓":"自用型保税仓";          
            //$list[$k]['cur_status'] = ($v['cur_status'] == '1' )?"启用":"停用"; 
    ...
    * */
     public function add()
    {      
         $this->load->view('wms/bus_record_add');
    }
    ...
    * */
    public function doadd(){
        ...
        $custom_name   = $this->input->get_post("custom_name");
		 if(!empty($custom_name)) {
            $condition['custom_name'] = $custom_name;
        }else {
            echo result_to_towf_new('1', 0, '关区名称不能为空', NULL);
            exit();
        }
		$enterprice_type   = $this->input->get_post("enterprice_type");
        $area_name    = $this->input->get_post("area_name");
        $org_code = $this->input->get_post("org_code");
        $enterprice_full_name    = $this->input->get_post("enterprice_full_name");
        $record_date = $this->input->get_post("record_date");
		$enterprice_ename    = $this->input->get_post("enterprice_ename");
		$register_date = $this->input->get_post("record_date");
        $valid_date = $this->input->get_post("valid_date");
		$creat_date = date('Y-m-d',time());
        //插入数据
        $data = array(
            'enterprice_code'      =>$enterprice_code,
            'enterprice_name'  =>$enterprice_name,
			'custom_name'   =>$custom_name,			
            'enterprice_type'=>$enterprice_type,
            'area_name'        =>$area_name,
			'org_code'   =>$org_code,			
            'enterprice_full_name'=>$enterprice_full_name,
            'record_date'       =>$record_date,
			'enterprice_ename'  =>$enterprice_ename,
        ...
        //echo $status;
        //break;
        $this->bus_record_model->insert($data);
    ...
    * */
    ...
    {
        $bus_record_id = $this->input->get_post("bus_record_id");		
		$condition = array();
        $condition['bus_record_id'] = $bus_record_id;
		//PC::debug($GUID);
		$list['info'] = $this->bus_record_model->get_list($condition);  
     // PC::debug($list);	
	    $this->load->view('wms/bus_record_edit', $list);
...
/**
    ...
    * */
     public function do_edit()
    {
       ...
       $this->bus_record_model->update($data,$bus_record_id);
	   echo result_to_towf_new('1', 1, 'success', null);
    }
    ...
    * */
    ...
    { 
		 ...
		 //PC::debug($GUID);
      if ($bus_record_id) {
            $this->bus_record_model->delete($bus_record_id);
